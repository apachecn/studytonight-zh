...
我们可以使用**构造函数**或者对象**文字**在 JavaScript 中创建一个对象。

## 创建一个 JavaScript 对象

我们可以通过使用`Object`构造函数或者使用对象文字语法来创建一个 JavaScript 对象。下面我们有一个创建对象的基本语法，它是使用两种方式的 JavaScript:
...
当我们使用对象构造器创建一个对象时，我们首先创建一个空对象，然后为其分配属性，而在对象文字语法的情况下，我们可以创建一个**空对象**或者在创建对象时在**大括号**内指定属性。
...
### 使用对象文字语法的 JavaScript 对象

使用对象文字语法创建对象，我们使用花括号`{ }`。这也是首选的方式，也是比较简单的方式，但是和`Object`构造器一样。我们可以在花括号内定义这个对象的属性。
