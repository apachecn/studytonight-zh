...
1.  A telecommunications organization, has offices spanned across multiple locations around the globe.
    ...
    ![minimum spanning tree example](../Images/370c29cab6e65ce6098b268512299e7f.png)图 1

2.  它必须使用租用的电话线来连接所有这些办公室。
3.  The cost(in units) of connecting each pair of offices is different and is shown as follows:

    ![minimum spanning tree example](../Images/54dc5cd313527d63880fd15f2db5eaf8.png)图 2
...
![spanning tree representation of graph](../Images/0b70db8fdc01990062648006a7596290.png)图 3

此外，对于任何给定的图，可能存在多个生成树。例如:除了上图中的生成树，该图还可以有另一个生成树，如下所示:

![spanning tree representation of graph](../Images/78dedcf7d3dd92be3ac5557df8bdb78b.png)图 4
...
![prim minimum spanning tree algo explanation](../Images/e5a917c95e4f1995014fce900c347ab8.png) <small>图 5:树为空时的初始数组</small>

### 第二步:

选择任意一个任意顶点，说出 **f** ，设置`key[f]=0`。

![set Key](../Images/1dca35204c390ca980af3e90f5f6c77d.png) <small>图 6:设置根节点键值</small>

由于其键值最小且未被访问，因此将 **f** 添加到生成树中。

![](../Images/6769ae6cbd3369bb49f4bfe738a02469.png) <small>图 7:根节点</small>
...
*   This is how the `visited[]` array will look like:
    ...
    ![visited array](../Images/265760f86ea3ace0e173e431a21f5a60.png) <small>图 8:添加根节点后的访问数组</small>
...
*   Key values for all the adjacent vertices of **f** will look like this(key value is nothing but the cost or the weight of the edge, for **(f,d)** it is still infinity because they are not directly connected):
    ...
    ![Key array](../Images/3e6bdd36feae4dd10b92ef2bf6b53060.png) <small>图 9:添加根节点后的键数组</small>

**注**:因为 **f** 是根节点，所以`parent[]`不会有变化。

![parent array](../Images/42b2d9edf81ff136dbc92820417044a6.png) <small>图 10:添加根节点后的父数组</small>
...
![adding vertex](../Images/a9868191a919b72e19049b3d51a07f9e.png) <small>图 11:添加顶点 c</small>
...
*   This is how the `visited[]` array will look like:
    ...
    ![visited array](../Images/99c2f0ed0cb91ac0232f82126020bed3.png) <small>图 12:添加顶点 c 后访问数组</small>

*   And, the `parent[]` array (**f** becomes the parent of **c**):

    ![parent array](../Images/fc8fca98d64ed9b3f246e7cecb3450be.png) <small>图 13:添加根节点后的父数组</small>
...
*   For every adjacent vertex of **c**, say **v**, values in `key[v]` will be updated using the formula:
    ...
    ![key array](../Images/ae611f069b38d66021bc8bbfdb6b3b4a.png) <small>图 14:添加根节点后的键数组</small>
...
*   Next vertex to be selected is **a**. And minimum cost will become `minCost=1+2=3`
    ...
    ![adding vertex](../Images/2fa41da1b31d5a144bd390061c85e433.png) <small>图 15:添加顶点 a</small>

    ![ key visited parent](../Images/c774640b0c3264e68e9d3f205956a971.png) <small>图 16:添加顶点 a 后更新数组</small>

*   Next, either **b** or **d** can be selected. Let's consider **b**. Then the minimum cost will become `minCost=3+3=6`

    ![adding vertex](../Images/5719b6a597ae84d0857c92bf320d638e.png) <small>图 17:给最小生成树添加顶点 b</small>

    ![ key visited parent](../Images/81894599ecfc860e9648c91f13048b3d.png) <small>图 18:添加顶点 b</small> 后更新数组

*   Next vertex to be selected is **d**, making the minimum cost `minCost=6+3=9`

    ![adding vertex](../Images/99b1b4aebf6a7c86bb4021a0ef443de0.png) <small>图 19:添加顶点 d</small>

    ![ key visited parent](../Images/684fe105a81aed398016f2f0a4a241e9.png) <small>图 20:添加顶点 d 后更新数组</small>

*   Then, **e** is selected and the minimum cost will become, `minCost=9+2=11`

    ![adding vertex](../Images/172607941ed72073e80d07ad470cf207.png) <small>图 21:添加顶点 e 这是最终的最小生成树</small>

    ![ key visited parent](../Images/eb662de4737afa373b643feeebdfec0b.png) <small>图 22:添加顶点 e 后更新的数组(最终数组)</small>
...
![output](../Images/f0351292486ddee4bc73877d7b64ab0d.png) <small>图 23:程序输出</small>
...
2.  用于近似求解旅行商问题、最小割问题等问题的算法。
    ...
    *   A **minimum cut problem** is used to find the minimum number of cuts between all the pairs of vertices in a planar graph. A graph can be classified as planar if it can be drawn in a plane with no edges crossing each other. For example,
        ...
        ![planer graph](../Images/ba572750e2a5f653551cb27a362dc9aa.png) <small>图 24:平面图</small>

3.  Also, a cut is a subset of edges which, if removed from a planar graph, increases the number of components in the graph

    ![planer graph](../Images/ba572750e2a5f653551cb27a362dc9aa.png) <small>图 25:平面图中的割集</small>
