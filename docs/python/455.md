...
在本教程中，我们将学习[蟒蛇](https://studytonight.com/python/getting-started-with-python)熊猫`Series.argsort()`法。此方法返回由对序列值进行排序的索引组成的序列。如果序列包含空值或缺失值，`Series.argsort()`方法给出`-1` 值作为其索引。
