# Pandas数据帧`interpolate()`方法

> 原文：<https://www.studytonight.com/pandas/pandas-dataframe-interpolate-method>

在本教程中，我们将学习Pandas`DataFrame.interpolate()`方法。此方法使用插值方法填充 NaN 值。带有多索引的数据帧支持`method='linear'`。当此方法应用于数据帧时，它通过填充空值返回序列或数据帧。如果`inplace=True`返回无。
