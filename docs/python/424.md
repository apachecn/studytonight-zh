...
在本教程中，我们将学习[Python](https://www.studytonight.com/python/getting-started-with-python)Pandas`DataFrame.nunique()` 的方法。此方法计算请求轴上不同或唯一观察的数量。它返回带有许多不同观察值的`Series`。
