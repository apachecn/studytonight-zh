...
在本教程中，我们将学习[Python](https://www.studytonight.com/python/getting-started-with-python)熊猫`DataFrame.cummin()`T4 法。它给出了数据框或系列轴上的累积最小值。它返回包含`cumulative minimum`的相同大小的数据帧或系列。

下图显示了 Python 熊猫`DataFrame.cummin()`方法的语法。
...
在本教程中，我们学习了 Python 熊猫`DataFrame.cummin()`方法。我们学习了语法、参数，通过求解例子，我们理解了`DataFrame.cummin()`方法。
