...
在本教程中，我们将学习[蟒蛇](https://www.studytonight.com/python/getting-started-with-python)熊猫`DataFrame.join()`的方法。此方法用于连接另一个数据框的列。它将列与索引或键列上的其他数据框连接起来。通过索引，该方法可以通过传递一个列表来连接多个数据框对象。
