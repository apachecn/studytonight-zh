...
在本教程中，我们将学习 Python Pandas`DataFrame.cumsum()`方法。它给出了数据帧或系列轴上的累计总和。它返回包含`cumulative sum`的相同大小的数据帧或系列。

下图显示了 Python Pandas`DataFrame.cumsum()`方法的语法。
...
在本教程中，我们学习了 Python Pandas`DataFrame.cumsum()`方法。我们学习了语法、参数，通过求解例子，我们理解了`DataFrame.cumsum()`方法。
