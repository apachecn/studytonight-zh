...
在本教程中，我们将学习[Python](https://www.studytonight.com/python/getting-started-with-python)熊猫`DataFrame.iterrows()`法。此方法以(索引、序列)对的形式迭代行。当此方法应用于数据框时，它会遍历数据框行并返回一个由`column name`和`content` 组成的`tuple`作为`Series`。
