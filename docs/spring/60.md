...
可以通过使用 XML 或 Java 注释来配置 Spring Security。我们已经在[上一篇文章](https://www.studytonight.com/spring-framework/basic-example)中看到了带有 Java 和注释的 Spring Security 配置。现在，我们将学习使用 XML 配置应用程序。如果您使用 java 或 XML，两者都很好，没有区别，但是在现代，使用基于 Java 的配置比使用 XML 更好。

让我们通过例子来理解。创建一个基于 maven 的 spring 应用程序，它将包含以下源文件。
...
它用于配置 web 应用程序。这里，我们设置 application-context.xml 和 spring-security.xml，以便应用程序可以在执行过程中找到它们。
...
![](../Images/4efa7fcff3a6d884253f3e3aed4666c8.png)

### 运行应用程序

成功完成项目并添加依赖项后，运行应用程序，您将获得如下输出。它呈现一个登录页面来验证用户。

它会将用户名和密码与 **spring-security.xml** 文件中提供的凭据进行匹配。

![](../Images/e4ebb597b413062798f3c6f56f682fd8.png)

### 提供正确的用户名和密码

![](../Images/1d9e2ac4776e441afb80a7d1416ce442.png)

### 主页

现在，您已成功登录到应用程序。这是我们的**index.jsp**文件渲染为浏览器的主页。

![](../Images/e46d60dfcff813baf7f806730bade8e8.png)

![](../Images/ddd912cdab669b34aa993a64e97031b6.png)

看，应用程序也可以很好地使用 XML 配置。因此，您可以为您的应用程序使用任何配置。
