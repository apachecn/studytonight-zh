...
在这篇关于 Spring Security 的文章中，我们将学习实现 RBAC(基于角色的访问控制)。RBAC 是一个主要用于企业应用程序的概念，其中多个用户正在访问资源，但资源受到限制。

假设一个应用程序包含几个模块，包括一个管理模块和一个用户模块，那么开发人员的职责就是只为管理用户提供一个管理模块。除管理员以外的任何用户都将被拒绝访问管理模块。
...
现在，我们已成功登录到应用程序。这是我们的**index.jsp**文件渲染为浏览器的主页。
...
![](../Images/95dac4fda651546724921aecb1b8d03e.png)
