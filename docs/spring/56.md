...
在本主题中，我们将学习根据用户角色显示数据。这意味着访客用户看不到管理员或任何其他角色的数据。当应用程序处理多个用户层次结构时，这很有用。例如，经理可以访问管理相关数据，但不能访问管理相关数据。

Spring 提供了一个[标签库](https://www.studytonight.com/spring-framework/spring-security-tag-library)，其中包含一个用于检查用户权限的授权标签。要了解如何将 Spring 标签库添加到我们的项目中，您可以参考[我们之前的主题](https://www.studytonight.com/spring-framework/user-information)。在这里，我们将看到如何实现它的 authorize 标签。下面给出了这个标签的一般语法。
...
![](../Images/c45c1cc57ab29ddb4aaf141d58d3fcb9.png)

### 登录另一个用户

![](../Images/2b17e736caafe7582d415b533b370053.png)

这次用户的角色是 REGISTERED，所以只能看到高级课程。

![](../Images/b239556ae4a1f7a8ffb5bfe7f1e5cc1e.png)
