...
在本文中，我们将学习在我们的 Spring 应用中使用百里香叶。百里香叶是一个模板引擎，用于在 Spring MVC 中创建视图页面。它提供了完整的 Spring 集成，旨在取代 JSP 成为 Spring MVC 中的视图页面。如果您正在使用网络应用，最好使用百里香叶。

它可以在网络和非网络应用中工作，甚至可以在离线环境中处理任何 XML 文件。要在您的项目中使用百里香叶，只需将这个依赖项添加到您的 **pom.xml** 文件中。
