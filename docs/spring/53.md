...
在上一个主题中，我们已经学习了在登录页面中添加一条[自定义错误消息](https://www.studytonight.com/tutorials/preview?subject=spring-framework&url=custom-error-message)。现在，我们将看到如何在我们的应用程序中实现注销功能，以便成功登录的用户也可以注销。
...
现在，将所有步骤合并到这个项目中，创建一个应用程序，允许登录、注销，并在单击注销按钮时给出正确的注销消息。请参见以下文件和源代码。
...
现在，您已成功登录到应用程序。这是我们的**index.jsp**文件渲染为浏览器的主页。看，它有一个**注销**按钮，我们添加到了**index.jsp**页面。
...
![](../Images/f4496341a8693ae45431ce168457c718.png)
