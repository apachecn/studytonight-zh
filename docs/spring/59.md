# Spring 安全 HTTP 基本身份验证

> 原文：<https://www.studytonight.com/spring-framework/spring-security-http-basic-authentication>

在本主题中，我们将学习在 Spring 应用程序中使用 HTTP 基本身份验证。Spring Security 提供 HTTP 基本身份验证，在客户端对用户进行身份验证，并将带有请求头的用户凭据发送到服务器。服务器接收这些凭证，从标题中提取它们，并将它们与现有记录映射以验证用户。

`BasicAuthenticationFilter`处理请求，检查请求是否包含认证头。`httpBasic()`方法在我们的应用程序中启用了基本的 HTTP 安全性。让我们创建一个项目并实现 HTTP 安全性。
...
我们在应用程序中使用`HttpSecurity`类来配置基本身份验证。`httpBasic()`方法用于在 HTTP 请求期间启用该功能对用户进行身份验证。
...
![](../Images/33402725d4d5857ac9e298da24a998d8.png)

### 提供正确的用户名和密码

![](../Images/04010a92de1cf190a3ef666992cb8ebf.png)

### 主页

现在，您已成功登录到应用程序。这是我们的**index.jsp**文件渲染为浏览器的主页。

![](../Images/64bc52025b0a0334319a6dbd1a9e9e02.png)
