...
`StringJoiner`类的工作方式与上面讨论的`join()`方法一样。它适用于 Java 8 及以上版本。我们需要初始化这个类的一个对象，并且需要向它的构造器传递一个分隔符。我们可以使用`add()`方法给它附加字符串。我们可以使用 toString()方法从`StringJoiner`对象中获取串联字符串。
