...
如果我们的类包含对其他对象的引用，那么我们还需要重写被引用对象类中的 toString()方法。

例如，让我们向学生类中添加一个地址类字段，并查看重写的 toString()方法的输出。
...
如我们所见，Address 类使用默认实现。我们得到地址类名和对象哈希码，但它不包括邮政编码。我们需要重写 Address 类中的 toString()方法来纠正这个问题。
...
Arrays 类还提供了一个 toString()方法来显示数组的内容。如果我们的类包含一个数组数据成员，我们必须在重写的 toString()方法中使用 **Arrays.toString()** 。让我们首先在不使用 Arrays.toString()的情况下查看输出。
...
许多预定义的类都有一个针对 toString()方法的重写实现。这是为了确保打印这些类的对象的有意义的字符串表示。如果我们有这些类的数据成员，我们就不需要在被重写的 toString()方法中使用任何其他方法。下面的代码演示了这一点。
...
对象类提供了一个 toString()方法，该方法返回 Java 对象的字符串表示形式。然而，这个方法在 Object 类中的实现不是很有用。由于这个原因，许多预定义的 Java 类，如整数、双精度、数组列表、字符串缓冲区覆盖了这个方法。在用户定义的 Java 类中重写这个方法也是一个很好的做法。
