...
`BufferedReader`类是`java.io`包的一部分，扩展了`Reader`类。BufferedReader 用于从字符`InputStream`中读取文本，并缓冲字符以获得更好的性能。
...
*   BufferedReader 包装另一个读取器或`InputStream`，以提高我们程序的整体效率和性能。
...
我们可以使用一个`InputStream`作为源来创建一个缓存器。
