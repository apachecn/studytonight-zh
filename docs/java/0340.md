...
枚举集是一种特殊类型的集合，旨在以高效的格式存储枚举常量。枚举使用**位向量表示**来存储枚举常数。EnumSet 可以执行所有常规的 Set 操作。在使用枚举类型时，总是建议首选枚举集而不是常规集。我们可以使用()方法创建所选常数的枚举。或者我们可以使用 allOf()方法来创建集合中所有常数的枚举。
...
我们可以向枚举类型添加一个字段或数据成员。该字段将与每个常数相关联。我们可以使用构造函数为每个常量初始化这个枚举字段。

例如，在下面的代码中，我们向枚举添加了一个字符串类型的百分比字段。我们还为这个字段编写了一个构造函数。所有枚举常数都用不同的参数调用了此构造函数。

请注意，枚举的构造函数总是私有的。我们不能使用任何其他修饰语。如果未指定访问修饰符，则隐式添加 Private。
...
*   枚举还可以包含其他字段和构造函数来初始化这些字段。
*   构造函数必须始终是私有的，我们不能使用 new 关键字来创建枚举的实例。
