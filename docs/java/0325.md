...
让我们创建一个存储整数-字符串对的树形图。当我们打印映射时，输出按排序顺序显示。
...
**可比界面**用于设置自定义类的自然顺序。让我们创建一个实现`Comparable`接口的学生类。我们需要按学生的姓名(字母顺序)对他们进行排序。TreeMap 将自动使用 compareTo()方法中定义的自然顺序对数据进行排序和存储。
