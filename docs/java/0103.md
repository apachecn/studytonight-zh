...
字节类是一个包装类，用于包装对象中基元类型字节的值。字节类型的对象包含类型为字节的单个字段。
...
让我们举个例子来获取字节类型的字符串对象。我们使用了`toString()`方法，它是静态的，所以我们可以用类名来调用它。
...
![byte-class](../Images/19391a61f0b0408a988a625ad2800d76.png)

### 2.valueOf()

此方法返回一个表示指定字节值的字节实例。此方法通常应优先于构造函数 Byte(字节)使用。
...
在这个例子中，我们使用`valueOf()`方法返回代表指定字节类型的字节类实例。
...
![byte-class-example](../Images/4a59d5c94e7b42bb155fb45311b2cb3b.png)

### 3.parseByte()

此方法返回指定字符串值的字节值。我们可以用它从字符串类型值中获取字节值。
...
让我们举一个例子，其中我们有一个字符串类型变量，并使用 parseByte()方法获取它的字节值。
...
![byte-class-example](../Images/00b26d7fa12ea49fdc388e3350c1106d.png)
...
我们可以使用 decode 方法将字符串类型解码为字节对象。见下面的例子。
...
![byte-class-example](../Images/87e7d55cbb94fa732b53540c5f300d66.png)
...
`longValue()`方法在扩展原语转换后，将该字节类型的值作为长类型返回。
...
让我们举一个例子，将字节类型转换为 int、long 和 float 类型的值。在这个例子中，我们使用`intValue(), floatValue(), doubleValue()`方法。
...
![byte-class-example](../Images/ba89677afd5be829318f59f4da1fce9a.png)
...
![byte-class-example](../Images/2800832bcda5d3f86f53a5af69704627.png)
...
![byte-class-example](../Images/f13fa9fcd2ab156c0b300753bbd8576d.png)
...
![byte-class-example](../Images/830e2738fb72f130bcea3b570829ed3e.png)
...
![byte-class-example](../Images/09caebf8ce138fa9f17bb4438a97aa8b.png)
