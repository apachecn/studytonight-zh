...
第一个给定的语法是针对二分搜索法的，当我们想要在整个数组中找到键的时候。我们将第一个参数作为我们想要搜索的数组传递，第二个参数将是我们想要搜索的关键字，第三个参数将是设置数组排序顺序的比较器。默认情况下，它会认为数组是按升序排序的。
...
| **静态< T >内部二进制搜索(T[] a，****int fromIndex，int toIndex，T 键，比较器<？超级 T > c)** | 此方法使用二分搜索法算法在指定数组的范围内搜索指定对象。 |
| **静态< T >内部二进制搜索(T[] a，****T 键，比较器<？超级 T > c)** | 此方法使用二分搜索法算法在指定的数组中搜索指定的对象。 |

## 示例:没有比较器的`binarySearch()`
...
## 示例:带比较器的`binarySearch()`

有时可能会发生数组以非递减方式排序的情况，在这种情况下，我们需要传递一个比较器函数，然后它将像正常的二分搜索法一样工作。在下面给出的代码中，您可以看到这个函数正在返回 33 的索引，它是正确的 4，而不考虑排序的顺序，只是因为比较器函数。
...
> ![enlightened](../Images/bcefbc0bebd753ed2a05f55c0b74d9f0.png "enlightened")二分搜索法总是按照排序的顺序工作，可能是升序，也可能是降序。它非常高效，因为它基于分治原则。
