...
在本教程中，我们将学习如何找到 nCr 的价值。nCr 值给出了从 n 个对象中选择 r 个对象的方法，不考虑顺序；更正式地说，n 元素集的 r 元素子集(或 r 元素组合)的数量。但是在继续之前，如果你不熟悉 java 中循环的概念，那么一定要查看关于 Java 中[循环](https://www.studytonight.com/java/loops-in-java.php)的文章。
...
## 程序 1:在 Java 中找到 nCr 值

在这个程序中，当值是用户定义的时，我们将找到 nCr 值。这意味着，首先我们将要求用户输入 n 和 r 值，然后我们将使用公式计算 nCr 值。这里，我们将使用 for 循环来计算阶乘。
...
9.  如果无法计算 nCr 值，则输入 n 和 r 的值，使 n>=r。
...
## 程序 2:在 Java 中找到 nCr 值

在这个程序中，当数值在程序中被预定义时，我们将找到 nCr 值。
...
8.  如果无法计算 nCr 值，则输入 n 和 r 的值，使 n>=r。
...
## 程序 3:在 Java 中找到 nCr 值

在这个程序中，当值是用户定义的时，我们将找到 nCr 值。这意味着，首先我们将要求用户输入 n 和 r 值，然后我们将使用公式计算 nCr 值。这里，我们将使用 while 循环来计算阶乘。
