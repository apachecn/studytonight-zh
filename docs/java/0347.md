...
Collections 框架提供了一种方便的 sort()方法来对元素进行排序。我们可以将哈希表中的键或值复制到`ArrayList`中，然后，我们可以使用 Collections.sort()方法轻松地对它们进行排序。

### 按关键字排序

我们可以通过使用 keySet()方法获得哈希表中存在的键。我们可以将这个键集传递给`ArrayList`构造器。这将使用哈希映射键初始化`ArrayList`。接下来，我们将简单地使用 sort()方法并打印排序后的键和相应的值。
...
当我们想要存储键值对时，HashMap 是一个方便的集合。在内部，它使用哈希表的概念。没有内置的方法对哈希表进行排序。但是，我们可以通过使用`ArrayList`和树集等其他集合来查看排序后的哈希表条目。如果您想保持地图条目的排序顺序，树形地图是一个很好的选择。我们还可以将排序后的键值对存储在 LinkedHashMap 中。LinkedHashMap 维护元素的插入顺序，我们只需要对地图排序一次。
