...
[Java](https://www.studytonight.com/java/overview-of-java.php) 中的 [HashMap](https://www.studytonight.com/java/hashmap-in-collection-framework.php) 类用于存储键值对的集合。实现[地图](https://www.studytonight.com/java/map-interface-in-java.php)界面。HashMap 不按排序顺序存储键值对，也不维护元素的插入顺序。

然而，有时我们希望以排序的方式查看存储的数据。我们可以根据键或值对哈希表进行排序。

在本教程中，我们将学习如何对哈希表进行排序。

## 使用集合对哈希表进行排序

Collections 框架提供了一种方便的 sort()方法来对元素进行排序。我们可以将哈希表中的键或值复制到数组列表中，然后，我们可以使用 Collections.sort()方法轻松地对它们进行排序。

### 按关键字排序

我们可以通过使用 keySet()方法获得哈希表中存在的键。我们可以将这个键集传递给数组列表构造函数。这将使用哈希映射键初始化数组列表。接下来，我们将简单地使用 sort()方法并打印排序后的键和相应的值。
...
同样，方法也是一样的。我们将首先使用 values()方法用 HashMap 值初始化一个数组列表。然后，我们可以使用 sort()方法对它们进行排序。我们将无法看到排序值的对应键。给定一个值，没有办法从 HashMap 中获取键。
...
如果您经常想查看按关键字排序的哈希表，那么树图将是比哈希表更好的选择。树形图自动按排序顺序存储键值对(按键排序)。我们可以将 HashMap 中的所有数据复制到一个 TreeMap 中，数据将被排序。我们可以使用 putAll()方法，也可以将 HashMap 传递给 TreeMap 构造函数。
...
哈希表必须包含唯一的键，但是对值没有限制。它可以包含重复的值。如果我们只想考虑一次重复的值，我们可以在 TreeSet 的帮助下按键对哈希表进行排序。就像 TreeMap 一样，TreeSet 也按排序顺序存储数据。我们需要将值从地图复制到集合中，就这样。
...
我们将使用 comparingByValue 比较器根据值对 HashMap 进行排序。
...
当我们想要存储键值对时，HashMap 是一个方便的集合。在内部，它使用哈希表的概念。没有内置的方法对哈希表进行排序。但是，我们可以通过使用数组列表和树集等其他集合来查看排序后的哈希表条目。如果您想保持地图条目的排序顺序，树形地图是一个很好的选择。我们还可以将排序后的键值对存储在 LinkedHashMap 中。LinkedHashMap 维护元素的插入顺序，我们只需要对地图排序一次。
