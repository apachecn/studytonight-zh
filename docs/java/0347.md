...
我们可以通过使用 keySet()方法获得哈希表中存在的键。我们可以将这个键集传递给数组列表构造器。这将使用哈希映射键初始化数组列表。接下来，我们将简单地使用 sort()方法并打印排序后的键和相应的值。
...
如果您经常想查看按关键字排序的哈希表，那么树图将是比哈希表更好的选择。树形图自动按排序顺序存储键值对(按键排序)。我们可以将 HashMap 中的所有数据复制到一个 TreeMap 中，数据将被排序。我们可以使用 putAll()方法，也可以将 HashMap 传递给 TreeMap 构造器。
