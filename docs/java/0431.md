...
考虑下面显示的例子。菱形运算符之间的字母 T 称为**类型参数**。可以使用字符串类型、整数类型甚至双精度来创建类对象。
...
考虑下面的代码，它使用 Generic print()方法来打印对象。<t>在方法定义中，告诉 Java 这是一个参数类型为 T 的 Generic 方法，我们可以在方法实现的任何地方使用这个类型 T。即使方法返回 void，也必须使用 diamond 运算符。</t>
...
就像泛型类一样，g **泛型方法也可以取多个类型参数**。例如，让我们创建一个方法，将任意类型的两个列表作为参数，它应该打印这两个列表。我们可以在菱形运算符中定义两种参数类型(T 和 E)。
...
例如，如果我们创建一个数组列表来存储整数，并且没有为其元素指定类型，那么下面的代码将成功编译。但是我们会在运行时得到一个错误。
...
泛型在编译时使用一种称为**类型擦除**的技术。类型擦除**将泛型的参数类型 T 替换为对象类(或者在泛型有界的情况下为父类)，并在编译时强制类型约束**。这确保了在运行时不需要做额外的工作，并减少了运行时开销。它确保不会为参数化类型创建新类。
...
由于类型擦除，**泛型不能与基元类型**一起工作。这是因为**原语类型没有扩展对象类**，或者任何其他类，因此在类型擦除过程中，编译器不能替换它们。但是我们总是可以使用包装类(比如整型和双精度型)来代替原语。
