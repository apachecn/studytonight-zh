...
| 链接列表 | 通过扩展`AbstractSequentialList`实现链表 |
...
*   `AbstractList`实现列表接口，因此它只能作为一个列表，而链接列表实现列表、去队列和`Queue`接口，因此它可以同时作为一个队列和列表。
...
4.  它创建一个使用`Hashtable`进行存储的集合。`Hashtable`通过一种叫做**哈希**的机制来存储信息。
