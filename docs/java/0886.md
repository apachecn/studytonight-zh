...
| 方法 | 描述 |
| --- | --- |
| `int read(byte[] b)` | 它用于从`InputStream`中读取字节数。 |
| `int read(byte[] b, int off, int len)` | 它用于从`InputStream`中读取`**len**` 字节的数据。 |
...
| `int skipBytes(int x)` | 它用于跳过`InputStream`中的`x` 字节数据。 |
| `String readUTF()` | 它用于读取使用 UTF-8 格式编码的字符串。 |
| `void readFully(byte[] b)` | 它用于从`InputStream`中读取字节，并将其存储到缓冲区[数组](https://www.javatpoint.com/array-in-java)中。 |
| `void readFully(byte[] b, int off, int len)` | 它用于从`InputStream`中读取`**len**` 字节。 |
