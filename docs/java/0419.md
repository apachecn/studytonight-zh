...
[Java 8](https://www.studytonight.com/java-8/) 推出了全新的[日期时间 API](https://www.studytonight.com/java-8/java-8-date-and-time-api) 。这个新的API的灵感来自流行的 JodaTime 库。添加它是为了克服现有日期API的缺点。
...
*   **其他改进** -新的 API 符合 ISO 标准，更容易理解。它提供了许多操作日期和时间的方法。与新的API相比，现有的API有点难以使用和理解。
...
新的日期和时间API旨在解决现有API的问题。新的日期和时间类是不可变的和线程安全的。本地日期、本地时间和本地日期时间类是新API最常用的类。这些新类出现在 java.time 包中。它还提供了一个 ZonedDateTime 类来处理时区。现有的API也缺乏这一功能。
