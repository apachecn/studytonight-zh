...
| 构造器 | 描述 |
| --- | --- |
| `BufferedInputStream(InputStream in)` | 它创建一个`BufferedInputStream`并保存它的参数，`InputStream``in`，以备后用。 |
| `BufferedInputStream(InputStream in, int size)` | 它用指定的缓冲区大小创建一个`BufferedInputStream`，并保存它的参数，`InputStream``in`，以备后用。 |
...
| 方法 | 描述 |
| --- | --- |
| `int available()` | 此方法返回可以从`InputStream`中读取的估计字节数，而不会被`InputStream`的下一个调用方法阻塞。 |
| `int read()` | 这个方法从`InputStream`中读取下一个字节的数据。 |
| `int read(byte[] b, int off, int ln)` | 此方法从给定的偏移量开始，将指定字节`InputStream`中的字节读入指定的字节数组。 |
| `void close()` | 此方法关闭`InputStream`并释放与该流关联的任何系统资源。 |
| `void reset()` | 此方法将流重新定位在此`InputStream`上最后一次调用 mark 方法的位置。 |
| `void mark(int readlimit)` | 此方法查看`InputStream`的 mark 方法的一般约定。 |
| `long skip(long x)` | 此方法跳过并丢弃`InputStream`中的 x 字节数据。 |
| `boolean markSupported()` | 

&#124; 该方法测试`InputStream`是否支持标记和重置方法。 &#124;
