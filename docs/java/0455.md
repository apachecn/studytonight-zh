...
以前的 Java 版本允许私有成员访问嵌套类(嵌套类)，但是我们不能将它们与反射API一起使用。Java 11 不再使用桥方法，而是为反射 API 提供了 getNestHost()、getnestmotof()和 isNestmatOf()方法。
...
HTTP 客户端API最初是在 Java 9 中引入的，并在 Java 10 中进行了更新。它是作为 Java 11 版本的标准功能提供的。新的 API 性能更好，兼容 HTTP/1.1 和 HTTP/2。该API还提供对网络套接字的支持。
