...
如果你不想把`ArrayList`转换成`Hashtable`，那么你可以使用另一种方法[蒸汽应用编程接口](https://www.studytonight.com/java-8/java-8-stream-api)。流提供了一个`distinct()`方法，从流中删除重复项，`toList()`方法返回一个唯一元素的列表。
