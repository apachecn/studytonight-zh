...
为了将数组列表转换为 LinkedHashSet，我们使用了各种方法，如[流 API](https://www.studytonight.com/java-8/java-8-stream-api) 或 add()方法或 LinkedHashSet 构造器。见下面的例子。
...
这是另一种从数组列表中获取 LinkedHashSet 的方法。这里，我们将数组列表作为参数传递给 LinkedHashSet 的构造器。
