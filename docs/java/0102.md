...
在 Java 中，Wrapper 类用于将原语数据类型转换为对象，并将对象转换为原语数据类型。对于每个原始数据类型，都有一个预定义的类，称为 Wrapper 类。从 J2SE 5.0 版本开始，自动装箱和拆箱功能用于将原始数据类型自动转换为对象，并将对象自动转换为原始数据类型。

### 为什么要使用包装类？

我们知道，在 Java 中，当用户给出输入时，它是以字符串的形式。为了将字符串转换成不同的数据类型，使用了 Wrapper 类。

当我们想要将原语转换为对象或者相反时，每次都可以使用包装类。
...
![wrapper-class](../Images/09d60659e252bd58fde40453ba89f6ea.png)
...
![number-class-example.](../Images/82ab1a740d7987f82d0914bc9a7721ac.png)
...
![integer-class](../Images/a9b295a7fb4e0eb337c512534e8ecbc2.png)
...
![integer-class-example](../Images/63a1cc34e8055814a553e719eb5365cf.png)
...
![integer-class-example](../Images/5078d6e676776f8dc1e3c49983e9c0c2.png)
...
![integer-class-example](../Images/7f552f97ef2d381b9d612447b4713738.png)
...
![integer-class-example](../Images/f60cb90b827af27a7c11927731da9310.png)

### 6.parseInt()

`parseInt()`方法用于将指定的字符串参数解析为带符号的十进制整数。字符串中的字符必须都是十进制数字。

它接受字符串类型的单个参数，并返回一个 int 值。
...
![integer-class-example](../Images/c546a51c543ff579130224efa2e6a096.png)
...
![integer-class-example](../Images/6c6c62540d71ca3759c7fa9b028a3706.png)

### 8.解码()

`decode()`方法用于将字符串解码为整数。它接受可以是十进制、十六进制和八进制数的字符串类型参数，并返回整数类的实例。下面给出了该方法的语法。
...
![integer-class-example](../Images/6ce7b4b853c75aa95b8769fab6b24da3.png)
...
![integer-class-example](../Images/99925a739b9f0fdcd9d615d5e27aedcb.png)
...
![long-class](../Images/d402be4713346b5afc62d8930abf2041.png)

### 2\. 十六进制字符串（）

`toHexString()`方法用于获取长参数的字符串表示。

此方法接受一个长类型的参数，并返回一个十六进制字符串。下面给出了该方法的语法。
...
当我们想要获得长类型值的八进制字符串表示时，这个方法很有帮助。

它可以用来获取长参数的字符串表示形式。

它接受一个长类型参数，并返回该参数的无符号字符串表示形式。下面给出了该方法的语法。
...
![long-class-example](../Images/4a422143d99919febef318c60748c3d4.png)

### 5.valueOf()

`valueOf()`方法用于获取表示指定长值的长对象。它接受一个长类型参数并返回一个长实例。下面给出了该方法的语法。
...
![long-class-example](../Images/9e5a50b1231c494ae9970b034a8c2079.png)

### 6\. 解析龙（）

`parseLong()`方法用于将指定的字符串参数解析为带符号的十进制长整型。字符串中的字符必须都是十进制数字。

它接受字符串类型的单个参数，并返回一个 Long 值。
...
![long-class-example](../Images/4aa0cbf1b4c4b1d82a06e53dc6940628.png)
...
![long-class-example](../Images/347afdf9079167d7d861789b5168a31f.png)

### 8 .解码()

`decode()`方法用于将字符串解码为长字符串。它接受可以是十进制、十六进制和八进制数的字符串类型参数，并返回 Long 类的实例。下面给出了该方法的语法。
...
![long-class-example](../Images/2a24afb6764f6f4ccf6afa8e6a4f77e0.png)
...
![long-class-example](../Images/32e16f7ddec0924b01cc0a92138cb4ee.png)
